...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        // used isset for array if not fullfill above both condition+
        if (isset($jobs)) {
...
//            $jobs['total'] = $totaljobs;
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            if ($data['immediate'] == 'no') {
                // move to "else if" because if any block call that will return response so rest of the part will be halt
                // so instead of single if for each block 'else if is fine'
            ...
            // this duration is common in else and imediate == 'no'
            // instead of over write move outside at once
            if (isset($data['duration']) && $data['duration'] == '') {
                $response['status'] = 'fail';
                $response['message'] = "Du måste fylla in alla fält";
                $response['field_name'] = "duration";
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
       ...
       // else if ($translator_type == 'volunteer') // $job_type Already unpaid
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        if (!empty($job->certified)) {
            if ($job->certified == 'yes' || $job->certified == 'both') {
                ...
                // indent and change "elseif" to "else if"
            } else if($job->certified == 'law' || $job->certified == 'n_law') {
                $translator_level[] = 'Certified with specialisation in law';
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    private function changeStatus($job, $data, $changedTranslator)
    {
        ...
        if ($old_status != $data['status']) {
            ...
            // added this block if status not changed then must send response
...
//        }
       // return false;
       // no need for this if condition is blocked
    }
...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout', 'assigned'])) {
        ...
        return true;
        // move this out of if block because in both condition return true
      ...
      //  else if ($translator_type == 'volunteer')
       //     $job_type = 'unpaid';  /* for volunteers only show unpaid jobs. */
        ...
        foreach ($job_ids as $k => $job) {
            ...
            //     unset($job_ids[$k]);
            // }
            // instead of above both if usinf below single if
